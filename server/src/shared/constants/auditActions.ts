/**
 * Audit action + resource catalog (SECURITY_PRINCIPLES.md §18).
 *
 * A centralized, reviewable string catalog rather than a Prisma enum: the set of
 * audited actions grows as the product does, and a plain `String` column + this
 * TS union avoids a schema migration for every new action while keeping the
 * values type-checked at the call sites. Mirrors the `roles.ts` catalog style.
 */

export const AUDIT_ACTIONS = {
  // Authentication
  AUTH_LOGIN_SUCCESS: 'AUTH_LOGIN_SUCCESS',
  AUTH_LOGIN_FAILED: 'AUTH_LOGIN_FAILED',
  AUTH_TOKEN_REFRESH: 'AUTH_TOKEN_REFRESH',
  AUTH_LOGOUT: 'AUTH_LOGOUT',
  // Two-factor authentication (email OTP)
  MFA_CHALLENGE_ISSUED: 'MFA_CHALLENGE_ISSUED',
  MFA_LOGIN_SUCCESS: 'MFA_LOGIN_SUCCESS',
  MFA_LOGIN_FAILED: 'MFA_LOGIN_FAILED',
  // User administration
  USER_CREATED: 'USER_CREATED',
  USER_UPDATED: 'USER_UPDATED',
  USER_ROLE_CHANGED: 'USER_ROLE_CHANGED',
  // Property management
  PROPERTY_CREATED: 'PROPERTY_CREATED',
  PROPERTY_UPDATED: 'PROPERTY_UPDATED',
  PROPERTY_DELETED: 'PROPERTY_DELETED',
  // Property-detail data groups (property-scoped sub-resources; audit field
  // NAMES / ids only, never sensitive values)
  UTILITY_BILL_CREATED: 'UTILITY_BILL_CREATED',
  UTILITY_BILL_UPDATED: 'UTILITY_BILL_UPDATED',
  UTILITY_BILL_DELETED: 'UTILITY_BILL_DELETED',
  EQUIPMENT_CREATED: 'EQUIPMENT_CREATED',
  EQUIPMENT_UPDATED: 'EQUIPMENT_UPDATED',
  EQUIPMENT_DELETED: 'EQUIPMENT_DELETED',
  GUARANTEE_CREATED: 'GUARANTEE_CREATED',
  GUARANTEE_UPDATED: 'GUARANTEE_UPDATED',
  GUARANTEE_DELETED: 'GUARANTEE_DELETED',
  EXPENSE_CREATED: 'EXPENSE_CREATED',
  EXPENSE_UPDATED: 'EXPENSE_UPDATED',
  EXPENSE_DELETED: 'EXPENSE_DELETED',
  INSPECTION_CREATED: 'INSPECTION_CREATED',
  INSPECTION_UPDATED: 'INSPECTION_UPDATED',
  INSPECTION_DELETED: 'INSPECTION_DELETED',
  // Foreign-worker management (records carry regulated PII — audit NAMES only)
  WORKER_CREATED: 'WORKER_CREATED',
  WORKER_UPDATED: 'WORKER_UPDATED',
  WORKER_DELETED: 'WORKER_DELETED',
  // Foreign-worker mobile-app authentication (phone/OTP/qrToken are NEVER logged —
  // audit reason codes / workerId only). Distinct from the staff AUTH_* actions.
  WORKER_AUTH_CHALLENGE_ISSUED: 'WORKER_AUTH_CHALLENGE_ISSUED',
  WORKER_AUTH_LOGIN_SUCCESS: 'WORKER_AUTH_LOGIN_SUCCESS',
  WORKER_AUTH_LOGIN_FAILED: 'WORKER_AUTH_LOGIN_FAILED',
  WORKER_AUTH_TOKEN_REFRESH: 'WORKER_AUTH_TOKEN_REFRESH',
  WORKER_AUTH_LOGOUT: 'WORKER_AUTH_LOGOUT',
  WORKER_AUTH_SESSION_REVOKED: 'WORKER_AUTH_SESSION_REVOKED',
  // Manager-driven app-access lifecycle for a worker principal
  WORKER_APP_ACCESS_ENABLED: 'WORKER_APP_ACCESS_ENABLED',
  WORKER_APP_ACCESS_DISABLED: 'WORKER_APP_ACCESS_DISABLED',
  WORKER_APP_ACCESS_QR_ROTATED: 'WORKER_APP_ACCESS_QR_ROTATED',
  // Worker-submitted requests (from the mobile app)
  WORKER_REQUEST_CREATED: 'WORKER_REQUEST_CREATED',
  // Worker identity documents (files are regulated PII — audit docType/id only,
  // never the original filename or bytes)
  WORKER_DOCUMENT_UPLOADED: 'WORKER_DOCUMENT_UPLOADED',
  WORKER_DOCUMENT_DOWNLOADED: 'WORKER_DOCUMENT_DOWNLOADED',
  WORKER_DOCUMENT_DELETED: 'WORKER_DOCUMENT_DELETED',
  // Property gallery images (audit propertyId/id only, never the original
  // filename or bytes)
  PROPERTY_IMAGE_UPLOADED: 'PROPERTY_IMAGE_UPLOADED',
  PROPERTY_IMAGE_DOWNLOADED: 'PROPERTY_IMAGE_DOWNLOADED',
  PROPERTY_IMAGE_DELETED: 'PROPERTY_IMAGE_DELETED',
  // Account lifecycle
  INVITATION_SENT: 'INVITATION_SENT',
  INVITATION_ACCEPTED: 'INVITATION_ACCEPTED',
  PASSWORD_RESET_REQUESTED: 'PASSWORD_RESET_REQUESTED',
  PASSWORD_RESET_COMPLETED: 'PASSWORD_RESET_COMPLETED',
  // Session
  // Emitted by refresh-token reuse detection (Batch 5): a replayed revoked/expired
  // token revokes the whole family + bumps tokenVersion. Also the action a future
  // admin disable/revoke endpoint should emit. Revocation triggered by a password
  // set/change is captured via `sessionsRevoked` metadata on the password events.
  SESSION_REVOKED: 'SESSION_REVOKED',
} as const;

export type AuditAction = (typeof AUDIT_ACTIONS)[keyof typeof AUDIT_ACTIONS];

export const RESOURCE_TYPES = {
  USER: 'USER',
  AUTH: 'AUTH',
  COMPANY: 'COMPANY',
  PROPERTY: 'PROPERTY',
  WORKER: 'WORKER',
  WORKER_AUTH: 'WORKER_AUTH',
  WORKER_REQUEST: 'WORKER_REQUEST',
  WORKER_DOCUMENT: 'WORKER_DOCUMENT',
  PROPERTY_IMAGE: 'PROPERTY_IMAGE',
  UTILITY_BILL: 'UTILITY_BILL',
  EQUIPMENT: 'EQUIPMENT',
  GUARANTEE: 'GUARANTEE',
  EXPENSE: 'EXPENSE',
  INSPECTION: 'INSPECTION',
} as const;

export type ResourceType = (typeof RESOURCE_TYPES)[keyof typeof RESOURCE_TYPES];
